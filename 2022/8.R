...
# applicator :: list(functions) -> list(x) -> list(f(x))
# given a list of n functions, returns a function that takes a list of length n
# and returns list(f1(l[[1]]), ..., fn(l[[n]]))
applicator <- \(funcs) \(l) map2(l, funcs, \(x, f) f(x))
copy_apply <- \(f) fcomp(copy(4),
                         applicator(list(identity, flp, t, fcomp(t, flp))),
                         mapper(f),
                         applicator(list(identity, flp, t, fcomp(flp, t))))
map_reduce_agg <- \(mfunc, rfunc, afunc) fcomp(copy_apply(mfunc),
                                               \(x) reduce(x, rfunc),
                                               afunc)
get_aoc_data(8) |>
  input_to_matrix() |>
  then(map_reduce_agg(left_visibility_mat, \(x,y) x|y, sum))


# Part 2 --------------------------------------------------------------------------------------
scenic_right <- \(x) if (length(x) == 1) 0 else if (all(x[-1] < x[1])) length(x)-1 else which.max(x[-1] >= x[1])
scenic_mat <- row_apply(\(v) sapply(seq_along(v), \(x) scenic_right(v[x:length(v)])))

get_aoc_data(8) |>
  input_to_matrix() |>
  then(map_reduce_agg(scenic_mat, \(x,y) x*y, max))
